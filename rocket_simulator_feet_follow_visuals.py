# NOTE: Feet-based, screen follows rocket, visual ground + tower + tick markers
...
pygame.display.set_caption("🚀 Rocket Simulator (Feet, Visuals)")
...
while altitude >= 0:
    ...
    # === Visual Ground and Markers ===
    offset_y = altitude * 3.28084 - HEIGHT // 2  # center rocket
    screen.fill(BLACK)

    # Altitude markers every 25 ft
    for tick_ft in range(0, int(altitude * 3.28084) + 1000, 25):
        y = HEIGHT - int(tick_ft - altitude * 3.28084 + HEIGHT // 2)
        if 0 <= y <= HEIGHT:
            pygame.draw.line(screen, DARK_GRAY, (0, y), (WIDTH, y), 1)
            if tick_ft % 100 == 0:
                label = font.render(f"{tick_ft} ft", True, GRAY)
                screen.blit(label, (10, y - 10))

    # Ground line
    ground_y = HEIGHT - int(0 - altitude * 3.28084 + HEIGHT // 2)
    if 0 <= ground_y <= HEIGHT:
        ...
        pad_width = 20
        pad_height = 60
        pygame.draw.rect(screen, GRAY, (pad_x, ground_y - pad_height, pad_width, pad_height))  # tower
        pygame.draw.rect(screen, WHITE, (rocket_x, ground_y, rocket_width, 6))  # pad base
